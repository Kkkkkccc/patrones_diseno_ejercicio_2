...
void test_strategy_sort_by_amount_desending() {
...
void test_decorator_audit_preserves_inner_export() {
    ...
    AuditedExporter<CsvExporter> exporter;
    auto output = exporter.export_expenses(expenses);
    assert(output.find("date,category,detail,amount") != std::string::npos);
    assert(output.find("transport") != std::string::npos);
    assert(output.find("AUDIT REPORT") != std::string::npos);
}
...
void test_decorator_summary_adds_total() {
    ...
    assert(output.find("21.70") != std::string::npos || output.find("21.7") != std::string::npos);
}
...
int main() {
    test_strategy_sort_by_amount_desending();
